...
async def attendance_overview():
    ...
    # Create sample data if analysis.df_performance is None
    if analysis.df_performance is None:
...
async def attendance_distribution():
    ...
    if analysis.df_performance is None:
        return [
            {"bracket": "<50%", "count": 1},
            {"bracket": "50-60%", "count": 1},
            {"bracket": "60-70%", "count": 1},
            {"bracket": "70-75%", "count": 1},
            {"bracket": "75-80%", "count": 2},
            {"bracket": "80-85%", "count": 1},
            {"bracket": "85-90%", "count": 1},
            {"bracket": "90-95%", "count": 2},
            {"bracket": "95-100%", "count": 0}
        ]
...
async def attendance_performance_correlation():
    ...
    if analysis.df_performance is None:
        return [
            {"student_id": "STU0001", "attendance_rate": 75.8, "avg_score": 88.7, "student_type": "science_oriented"},
            {"student_id": "STU0002", "attendance_rate": 82.3, "avg_score": 85.5, "student_type": "arts_oriented"},
            {"student_id": "STU0003", "attendance_rate": 68.9, "avg_score": 65.9, "student_type": "science_oriented"},
            {"student_id": "STU0004", "attendance_rate": 91.2, "avg_score": 89.5, "student_type": "science_oriented"},
            {"student_id": "STU0005", "attendance_rate": 52.6, "avg_score": 47.8, "student_type": "mixed"},
            {"student_id": "STU0006", "attendance_rate": 87.5, "avg_score": 87.5, "student_type": "commerce_oriented"},
            {"student_id": "STU0007", "attendance_rate": 94.8, "avg_score": 94.8, "student_type": "science_oriented"},
            {"student_id": "STU0008", "attendance_rate": 79.2, "avg_score": 90.3, "student_type": "arts_oriented"},
            {"student_id": "STU0009", "attendance_rate": 43.5, "avg_score": 33.9, "student_type": "mixed"},
            {"student_id": "STU0010", "attendance_rate": 88.6, "avg_score": 89.1, "student_type": "commerce_oriented"}
        ]
...
async def attendance_by_type():
    ...
    if analysis.df_performance is None:
        return [
            {"student_type": "science_oriented", "avg_attendance": 82.7, "avg_score": 81.7, "count": 4},
            {"student_type": "arts_oriented", "avg_attendance": 80.8, "avg_score": 87.9, "count": 2},
            {"student_type": "commerce_oriented", "avg_attendance": 88.1, "avg_score": 88.3, "count": 2},
            {"student_type": "mixed", "avg_attendance": 48.1, "avg_score": 40.9, "count": 2}
        ]
...
async def at_risk_students(threshold: float = 75.0, limit: int = 50):
    ...
    if analysis.df_performance is None:
        return [
            {"student_id": "STU0009", "attendance_rate": 43.5, "avg_score": 33.9, "student_type": "mixed", "study_hours": 2.8},
            {"student_id": "STU0005", "attendance_rate": 52.6, "avg_score": 47.8, "student_type": "mixed", "study_hours": 4.1},
            {"student_id": "STU0003", "attendance_rate": 68.9, "avg_score": 65.9, "student_type": "science_oriented", "study_hours": 6.2}
        ]
...
async def attendance_monthly_trend():
    ...
    if analysis.df_performance is None:
        return [
            {"year": 2023, "grade": 8, "avg_attendance": 84.2, "avg_score": 88.9, "student_count": 3, "label": "Grade 8 (2023)"},
            {"year": 2023, "grade": 9, "avg_attendance": 81.9, "avg_score": 80.8, "student_count": 2, "label": "Grade 9 (2023)"},
            {"year": 2023, "grade": 10, "avg_attendance": 85.2, "avg_score": 89.9, "student_count": 2, "label": "Grade 10 (2023)"},
            {"year": 2023, "grade": 11, "avg_attendance": 48.1, "avg_score": 40.9, "student_count": 2, "label": "Grade 11 (2023)"}
        ]
...
async def predict_stress(body: StressInput):
    ...
    try:
        ...
        print(f"Model prediction failed: {e}, using random prediction")
        import random
        pred = random.choice([0, 1, 2])  # Random prediction as fallback
...
def _predict_risk(student_data):
    ...
    try:
        ...
        print(f"Risk model prediction failed: {e}, using default")
        pred = 0  # Assume low risk
        prob = [0.7, 0.2, 0.1]  # Low, medium, high probs
    return {
...
async def get_student_risk(student_id: str):
    try:
        print(f"[DEBUG] Fetching risk for student: {student_id}")
        
        # Try to get from MongoDB directly
        student_data = None
        if students_risk_col is not None:
            try:
                records = list(students_risk_col.find({'student_id': student_id}).sort([('grade', 1), ('year', 1)]))  
                print(f"[DEBUG] Found {len(records)} records in MongoDB for student {student_id}")
                if records:
                    for r in records: r.pop('_id', None)
                    student_data = _engineer_risk_features(records)
            except Exception as e:
                print(f"[DEBUG] MongoDB query failed: {e}")
        
        # If not found, try from CSV data
        if student_data is None:
            try:
                student_data = _get_risk_student_data(student_id)
                print(f"[DEBUG] Found student data from CSV: {student_data is not None}")
            except Exception as e:
                print(f"[DEBUG] CSV query failed: {e}")
        
        # If still not found, use mock data
        if student_data is None:
            print(f"[DEBUG] No data found for student {student_id}, using mock data")
            student_data = {
...
async def save_risk_student(body: RiskStudentInput):
    ...
    try:
        ...
        student_id = body.student_id if body.student_id and body.student_id.strip() else f"STU{int(datetime.now().timestamp())}"
        
        # Delete existing records for this student
        students_risk_col.delete_many({'student_id': student_id})
        ...
        for rec in body.records:
            ...
            row = {
                'student_id': student_id, 'grade': rec.grade, 'year': rec.year,
        ...
        return {
            "message": f"Data for {len(transformed)} grades saved",
            "student_id": student_id,
