...
REM ── 1. Learning Path Backend (FastAPI) ──────────────────────
echo Starting Learning Path API (port 8000)...
start "Learning Path API" cmd /k "cd /d %ROOT%backend\learning_path && pip install -r requirements.txt -q && uvicorn main:app --reload --port 8000"
