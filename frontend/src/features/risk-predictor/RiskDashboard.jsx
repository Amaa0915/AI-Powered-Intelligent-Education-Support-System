            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              {onViewHistory && (
                <button type="button" onClick={onViewHistory}
                  className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-xl font-semibold text-sm transition-all border border-slate-200">
                  📋 View History
                </button>
              )}
            </div>
...
const RiskDashboard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URLS.RISK_PREDICTOR_BACKEND}/api/risk/${studentId}`)
  ...
  return (
    <>
      <HistoryModal
        ...
        title="Risk Prediction History"
        loading={historyLoad}
        error={historyErr}
        records={historyRecs}
        emptyText="No risk assessments saved for this student yet."
        columns={["#", "Date", "Risk Level", "Probability", "Avg Score"]}
        renderRow={(rec, i) => {
          const lvl = rec.risk_assessment?.risk_level || "Unknown";
          const color = lvl === "Low Risk" ? "#10b981" : lvl === "Medium Risk" ? "#f59e0b" : "#ef4444";
          const prob = rec.risk_assessment?.probabilities?.high || 0;
          return (
            <>
              <td className="px-4 py-3 text-xs text-slate-500">{i + 1}</td>
              <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">{fmtDate(rec.predicted_at || rec.timestamp || new Date().toISOString())}</td>
              <td className="px-4 py-3">
                <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                  style={{ background: `${color}18`, color, border: `1px solid ${color}30` }}>{lvl}</span>
              </td>
              <td className="px-4 py-3 text-xs text-slate-500">{(prob * 100).toFixed(1)}%</td>
              <td className="px-4 py-3 text-xs text-slate-500">{(rec.metrics?.academic_performance?.value || 0).toFixed(1)}%</td>
            </>
          );
        }}
      />
